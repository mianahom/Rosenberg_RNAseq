...
#read in meta data
meta <- read.csv("Colon_Biopsy_RNASeq_Metadata.csv.csv") %>%
...
plotCounts(distal_dds,"ENSG00000077942",intgroup="urolithin")
...
dr1_df <- distal_res1 %>% as.data.frame()
dr1_df <- merge(x=dr1_df,y=humangenes, by.x=0, by.y="Gene.stable.ID", all.x=TRUE)
names(dr1_df)[1]<-paste("Ensembl_ID")
dr1_df <- dr1_df %>% arrange(padj)
write.csv(dr1_df,file="distal_High_to_Low.csv")

dr2_df <-distal_res2 %>% as.data.frame()
dr2_df <- merge(x=dr2_df,y=humangenes, by.x=0, by.y="Gene.stable.ID", all.x=TRUE)
names(dr2_df)[1]<-paste("Ensembl_ID")
dr2_df <- dr2_df %>% arrange(padj)
write.csv(dr2_df,file="distal_Med_to_Low.csv")

dr3_df <-distal_res3 %>% as.data.frame()
dr3_df <- merge(x=dr3_df,y=humangenes, by.x=0, by.y="Gene.stable.ID", all.x=TRUE)
names(dr3_df)[1]<-paste("Ensembl_ID")
dr3_df <- dr3_df %>% arrange(padj)
write.csv(dr3_df,file="distal_High_to_Med.csv")

pr1_df <-proximal_res1 %>% as.data.frame()
pr1_df <- merge(x=pr1_df,y=humangenes, by.x=0, by.y="Gene.stable.ID", all.x=TRUE)
names(pr1_df)[1]<-paste("Ensembl_ID")
pr1_df <- pr1_df %>% arrange(padj)
write.csv(pr1_df,file="proximal_High_to_Low.csv")

pr2_df <-proximal_res2 %>% as.data.frame()
pr2_df <- merge(x=pr2_df,y=humangenes, by.x=0, by.y="Gene.stable.ID", all.x=TRUE)
names(pr2_df)[1]<-paste("Ensembl_ID")
pr2_df <- pr2_df %>% arrange(padj)
write.csv(pr2_df,file="proximal_Med_to_Low.csv")

pr3_df <-proximal_res3 %>% as.data.frame()
pr3_df <- merge(x=pr3_df,y=humangenes, by.x=0, by.y="Gene.stable.ID", all.x=TRUE)
names(pr3_df)[1]<-paste("Ensembl_ID")
pr3_df <- pr3_df %>% arrange(padj)
...
dr3_lfcorder <- data.frame(distal_res_shrink3) %>%
  filter(!is.na(padj)) %>% 
  arrange(-abs(log2FoldChange)) %>% 
  rownames() 
...
pheatmap(
  as.matrix(distalr1_heatmap_metrics[,2:18]), 
  cluster_rows=TRUE, 
  show_rownames=TRUE,
  cluster_cols=TRUE,
  labels_row=distalr1_heatmap_metrics$Gene.name,
...
#  arrange(-abs(log2FoldChange)) %>% 
#  rownames() 
assay <- assay(distal_rld) 
assay_genenames <-merge(x=assay,y=humangenes, by.x=0, by.y="Gene.stable.ID", all.x=TRUE)
assay_genenames_dr2 <-assay_genenames[assay_genenames$Row.names %in% dr2_lfcorder[1:30],]
assay_genenames_dr1 <-assay_genenames[assay_genenames$Row.names %in% dr1_lfcorder[1:30],]
assay_genenames_dr3 <-assay_genenames[assay_genenames$Row.names %in% dr3_lfcorder[1:30],]
  #distalr2_heatmap_metrics <- merged_data_distal[merged_data_distal$Row.names %in% dr2_lfcorder[1:30],]
# use regularized log-scaled counts
pheatmap(
  as.matrix(assay_genenames_dr3[,2:18]), 
  cluster_rows=TRUE, 
  show_rownames=TRUE,
  cluster_cols=TRUE,
  labels_row=assay_genenames_dr3$Gene.name,
  annotation_col=dr_dataframe
)
